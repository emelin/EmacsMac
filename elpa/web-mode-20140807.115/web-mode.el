...
(defun web-mode-tag-beginning ()
  ...
  (interactive)
  (let (pos)
    (setq pos (web-mode-tag-beginning-position (point)))
...
(defun web-mode-tag-end ()
  ...
  (interactive)
  (let (pos)
    (setq pos (web-mode-tag-end-position (point)))
    (when pos
      (setq pos (1+ pos))
      (goto-char pos))
    pos))
...
(defun web-mode-tag-previous ()
  ...
  (interactive)
  (let (pos)
    (setq pos (web-mode-tag-previous-position (point)))
    (when pos (goto-char pos))
    pos))
...
(defun web-mode-test-process (file)
  (with-temp-buffer
    (let (out sig1 sig2 success)
      ...
      (message out)
      (when (not success)
        (write-file (concat (file-name-directory file) "_err." (file-name-nondirectory file)))
